...
将论文 PDF 自动整理为“按图梳理”的结构化 Markdown 笔记（中文优先）。
...
2. 解压 ZIP，将解压得到的**整个技能目录**放到：
   - `C:\Users\<you>\.codex\skills\`
3. 如果解压后的目录名是 `paper-pdf-flow-main`，请重命名为 `paper-pdf-flow`。
4. 确认最终路径满足以下两个条件：
   - `C:\Users\<you>\.codex\skills\paper-pdf-flow\SKILL.md`
   - `C:\Users\<you>\.codex\skills\paper-pdf-flow\scripts\pdf_to_flow_note.py`
5. 安装依赖：
   - pip install pypdf
6. 重启/刷新 Codex 或 Claude Code 会话。
7. 在对话里调用（把尖括号内的路径换成自己的）：
   ```text
