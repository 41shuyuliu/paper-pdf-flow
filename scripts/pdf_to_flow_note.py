...
def zh_caption_summary(caption: str) -> str:
    ...
    if not tags:
        return "该图展示关键实验步骤或结果（建议结合原图注人工复核）。"
    return "该图主要展示：" + "、".join(tags) + "。"
...
def zh_problem_lines(figs: List[Tuple[str, str]], signals: Dict[str, List[str]]) -> List[str]:
    ...
    if not lines:
        lines.append("- 论文关注如何把方法、验证与结论串成可复现的完整流程。")
    lines.append("- 目标：建立可规模化、可追踪、可定量的图驱动实验流程。")
    return lines[:4]
...
def zh_fallback_flow_lines(full_text: str) -> List[str]:
    ...
    if any(k in text for k in ["method", "approach", "architecture", "network", "model"]):
        lines.append("- 论文首先给出模型/方法框架与关键模块设计。")
    else:
        lines.append("- 论文首先定义任务目标与总体技术路线。")
    ...
    if any(k in text for k in ["dataset", "data set", "training", "implementation", "preprocess"]):
        lines.append("- 接着说明数据来源、训练配置与实现细节。")
    else:
        lines.append("- 接着说明实验数据、对照设置与评估方案。")
    ...
    if any(k in text for k in ["result", "comparison", "ablation", "experiment"]):
        lines.append("- 随后通过对比实验与（或）消融实验验证方法有效性。")
    else:
        lines.append("- 随后给出定量与定性结果，验证方法表现。")
    ...
    if any(k in text for k in ["conclusion", "discussion"]):
        lines.append("- 最后总结方法优势、适用边界与潜在改进方向。")
    else:
        lines.append("- 最后总结主要贡献与局限。")
...
def zh_fig_bullets(tags: List[str]) -> List[str]:
    bullets: List[str] = []
    if "流程/装置示意" in tags:
        bullets.append("- 先交代整体实验流程与系统搭建。")
    if "生物打印构建" in tags:
        bullets.append("- 通过标准化生物打印提升样本构建一致性。")
    if "Matrigel 几何与厚度优化" in tags:
        bullets.append("- 通过几何/厚度优化提升成像可追踪性。")
    if "细胞活性验证" in tags:
        bullets.append("- 验证构建步骤不会显著损伤细胞活性。")
    if "转录组/分子一致性对比" in tags:
        bullets.append("- 从分子层面对比基线与改进流程的一致性。")
    if "单类器官追踪与动态成像" in tags:
        bullets.append("- 对单个类器官进行连续追踪，得到时间序列变化。")
    if "药物响应分析" in tags:
        bullets.append("- 对不同处理条件进行动态药物响应对比。")
    if "耐药/敏感亚群与异质性" in tags:
        bullets.append("- 识别耐药/敏感亚群，评估样本内异质性。")
    if "终点 ATP 对照" in tags:
        bullets.append("- 与终点法对照，验证动态方法的增益。")
    if not bullets:
        bullets.append("- 该图用于补充关键流程或结果。")
    return bullets[:4]
...
def zh_final_strict_template(
    ...
    if figs:
        ...
        fig_header = f"## 按图看整篇流程（Fig.1~Fig.{max_fig}）"
        idx = 1
        ...
        fig_header = "## 按图看整篇流程（Fig.1~Fig.N）"
